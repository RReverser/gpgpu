        ...
        if (typeof prop === 'function') {
            return new Proxy(prop, {
                apply: function (targetFn, thisArg, args) {
                    ...
                    console.log.apply(console, [newResult ? newName + " =" : '', results.get(thisArg), ". " + name + "("].concat(args.map(function (arg, i) {
                        return results.get(arg) || (typeof arg !== 'object' && typeof arg !== 'undefined' ? JSON.stringify(arg) : arg);
                    }).reduce(function (res, arg, i) {
...
if (!gl)
    throw ("Requires WebGL rendering context!");
...
function createFloatingPointTextureFromData(data, size) {
    if (size === void 0) { size = findBestTextureSize(data.length); }
...
function findBestTextureSize(length) {
    ...
    var size;
    if (sqrt > MAX_TEXTURE_SIZE)
        throw new TypeError('Error: Too Much Data. Multiple texure buffers are not yet supported.');
    else if (sqrt > 8192)
        size = 16384;
    else if (sqrt > 4096)
        size = 8192;
...
function gpgpu(_a, data) {
    var vertex = _a.vertex, fragment = _a.fragment;
